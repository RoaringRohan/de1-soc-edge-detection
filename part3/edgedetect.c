...
void flip(struct pixel *data, int width, int height)
{
    ...
    for (i = 0; i < height / 2; ++i)
        for (j = 0; j < width; ++j)
        {
            ...
            // Swapping pixel at row i with pixel at opposite row (height-1-i)
            tmp = image[i][j];
...
void memcpy_consecutive_to_padded(struct pixel *from, volatile unsigned int *to, int pixels)
{
    ...
    int i;
    unsigned int red, green, blue, pixel_val;

    for (i = 0; i < pixels; i++)
    {
        // Store colour components from pixel struct
        red = from[i].r;
        green = from[i].g;
        blue = from[i].b;

        // Each pixel in memory must be word-aligned
        // Every 32 bits in memory stores one pixel
        // 31-24 are padding, 23-16 red, 15-8 green, 7-0 blue
        pixel_val = (red << 16) | (green << 8) | blue;

        // Write the 32-bit value to the SDRAM memory
        to[i] = pixel_val;
    }
}
...
void memcpy_padded_to_consecutive(volatile unsigned int *from, struct pixel *to, int pixels)
{
    ...
    for (i = 0; i < pixels; i++)
    {
        // Read 32-bit value from hardware memory SDRAM
        // Every 32 bits in memory stores one pixel
        // 31-24 are padding, 23-16 red, 15-8 green, 7-0 blue
        pixel_val = from[i];

        // Extract and assign colour channels
        to[i].r = (pixel_val >> 16) & 0xFF;
        to[i].g = (pixel_val >> 8) & 0xFF;
        to[i].b = pixel_val & 0xFF;
    }
}
